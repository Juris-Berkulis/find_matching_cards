...
module.exports = {
    ...
    plugins: [
        ...
        new WorkboxPlugin.GenerateSW({ //* Подробная информация о настройках плагина для service worker'а на сайте: "https://developer.chrome.com/docs/workbox/reference/workbox-build/#type-WebpackGenerateSWOptions".
            ...
            exclude: [/robots.txt/], //* Один или несколько спецификаторов, используемых для исключения ресурсов из манифеста предварительного кэширования. Это интерпретируется по тем же правилам, что и стандартная опция исключения webpack. Если не указано, значение по умолчанию — [/\.map$/, /^manifest.*\.js$].
